""" HAT-P-20b: Mont4K 2018 Light Curve Fit
...
a[0].plot(time, model_flux_time, 'r-', label ="BATMAN Model Flux")
...
plt.savefig(f'premcmc/HAT-P-20b_PreMCMC_LC.png')
...
def RunMCMC(p0,nburn,nprod,nwalkers,ndim,log_probability,args):
    ...
    np.savez(f'HAT-P-20b_MCMC_Results', chain=sampler.chain, flatchain=sampler.flatchain, lnprob=sampler.lnprobability, flatlnprob=sampler.flatlnprobability)
    ...
    labels = ['$t_0$','cos(i)','$R_{p}/R_{*}$','$log(a/R_{*})$','log(P)'] #,'log(a)','log(c)','$\sqrt{e}sin(\omega)}$','$\sqrt{e}cos(\omega)}$','$M_{p}/M_{*}$','$\gamma$','$dv/dt$','$T_{eff}$','$R_{*}$','Parallax','A-V']
    for i in range(5):
        ax = a[i]
    ...
    plt.savefig(f"mcmc/HAT-P-20b_MCMC_WP.png")
    # Fit Parameters
    pars_fit = np.zeros(5)
    ...
    figure = corner.corner(flatchain, labels = ['$t_0$','cos(i)','$R_{p}/R_{*}$','$log(a/R_{*})$','log(P)'], label_kwargs=dict(fontsize=20), truths=[pars_fit[0], pars_fit[1], pars_fit[2], pars_fit[3], pars_fit[4]], truth_color='#ff0000', quantiles=[0.16, 0.5, 0.84], top_ticks=True, verbose=False)
    plt.savefig(f"mcmc/HAT-P-20b_MCMC_Corner.png")
